...
func newMockUserMongoRepository(collection *mongo.Collection) repositories.UserRepository {
	nopLogger := czap.New(
...
func (s *MongoRepositoryTestSuite) TestUserCreation() {
	...
	// Start test
	userData := user.New("nick", "Ivan", "Petrov")
	credential := credential.New("login", "hashedPassword", &userrole.User)
